...
(async () => {
  const BACKFILL_SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || '';
  const BACKFILL_SUPABASE_SERVICE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || '';

  if (!BACKFILL_SUPABASE_URL || !BACKFILL_SUPABASE_SERVICE_KEY) {
  ...
  async function fetchRecordsToFill() {
    ...
    const response = await fetch(
      `${BACKFILL_SUPABASE_URL}/rest/v1/job_descriptions?${query}`,
      {
        headers: {
          'apikey': BACKFILL_SUPABASE_SERVICE_KEY,
          'Authorization': `Bearer ${BACKFILL_SUPABASE_SERVICE_KEY}`,
        },
      }
    );
  ...
  async function updateRecord(id: number, company: string) {
    const response = await fetch(
      `${BACKFILL_SUPABASE_URL}/rest/v1/job_descriptions?id=eq.${id}`,
      {
        method: 'PATCH',
        headers: {
          'apikey': BACKFILL_SUPABASE_SERVICE_KEY,
  ...
  async function main() {
    ...
    // 2. 提取公司名
    const results = records.map((record: { id: number; job_title: string; company: string | null; raw_jd: string }) => {
