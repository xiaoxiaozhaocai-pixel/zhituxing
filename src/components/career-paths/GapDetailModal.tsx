      ...
      >
        ...
        <div className="p-5 border-b border-slate-100">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold text-slate-900 text-base">差距分析</h3>
            <button onClick={onClose} className="text-slate-400 hover:text-slate-600 text-xl leading-none">&times;</button>
          </div>
          <p className="text-sm text-slate-500 mt-1">{routeName}</p>
        ...
        <div className="p-5 space-y-4">
          {gaps.length === 0 ? (
            <div className="text-center py-6 text-green-600">
              ...
              <p className="text-sm text-slate-500 mt-1">你没有需要提升的地方</p>
            </div>
          ) : (
            gaps.map((gap, i) => (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-slate-800 text-sm">{gap.label}</span>
                      <span className="text-xs px-1.5 py-0.5 rounded bg-red-100 text-red-600">不满足</span>
                    </div>
                    <div className="text-xs text-slate-500 mt-1">
                      要求：<span className="text-slate-700">{gap.required}</span>
                      &nbsp;·&nbsp;你的：<span className="text-slate-700">{gap.current}</span>
                    </div>
        ...
        <div className="p-4 border-t border-slate-100 text-center">
          <button
            onClick={onClose}
            className="px-6 py-2 text-sm rounded-lg bg-gray-100 text-slate-600 hover:bg-gray-200 transition"
          >
            关闭
          </button>
        </div>
