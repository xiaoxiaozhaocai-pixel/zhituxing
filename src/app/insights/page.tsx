...
export const metadata: Metadata = {
  ...
  description:
    '职途星求职判断力内容库免费开放：覆盖全行业的面试行业雷达、JD/简历/面试/职场潜台词词条翻译、专业认知库、岗位能力词典与判断力因果层（专业≠岗位的学术实证依据）。来自真实招聘洞察，求职先想清楚，再投简历。',
  keywords: [
    '求职判断力',
    '面试行业雷达',
    'JD潜台词',
...
// P2-1：内容→行动导流。每个板块底部「去试试」，把「看完（知道）」转成「去用（行动）」。
...
export default function InsightsPage() {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `专业认知库（${ALL_COGNITIVE_KNOWLEDGE.length}个专业）` },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#f8fafd] via-white to-[#f0f5ff]/40">
      ...
      />
      {/* 顶部 Hero（精简版：一行副标题说清价值，板块数据由 Tab 承载） */}
      ...
      {/* 五大板块 Tab 切换（SSR 全量渲染，非激活面板视觉隐藏，hash 直链兼容） */}
      <InsightsTabs panels={panels} />
