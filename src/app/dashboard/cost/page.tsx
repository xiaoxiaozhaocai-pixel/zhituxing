...
interface CostTooltipPayloadItem {
  name: string;
  value: number;
  color: string;
}
...
/* ─── 自定义 Tooltip ─── */
function CostTooltip({ active, payload, label }: { active?: boolean; payload?: CostTooltipPayloadItem[]; label?: string }) {
  ...
  return (
    <div className="bg-white border border-[#E2E8F0] shadow-lg rounded-lg p-3 text-sm">
      ...
      {payload.map((entry: CostTooltipPayloadItem, i: number) => (
...
export default function CostDashboardPage() {
  ...
  // 图表数据
  const chartData = detail.map((d: CostDetailItem) => ({
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {detail.map((row: CostDetailItem, i: number) => {
