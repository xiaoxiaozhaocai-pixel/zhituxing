...
export async function GET(request: NextRequest) {
  ...
  // 将 session 写入浏览器 cookie，否则用户点击邮件链接后验证"成功"却拿不到登录态
  const redirectWithSession = (
    ...
    if (session?.access_token && session?.refresh_token) {
      setAuthCookies(response, session.access_token, session.refresh_token, session.expires_at ?? 0);
    }
    return response;
  };
  ...
  if (code) {
    try {
      ...
      return redirectWithSession(data.session, safeNext);
    } catch (err: unknown) {
