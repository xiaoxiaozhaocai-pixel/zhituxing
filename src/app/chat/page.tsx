...
function trStr(v: unknown, fallback = ''): string {
  if (typeof v === 'string') return v;
  if (typeof v === 'number' && Number.isFinite(v)) return String(v);
  return fallback;
}
function trArr(v: unknown): unknown[] {
  return Array.isArray(v) ? v : [];
}
function trNum(v: unknown): number | null {
...
/** 工具结果分数徽标 */
function ToolScoreBadge({ score }: { score: number }) {
  const color =
    score >= 80
