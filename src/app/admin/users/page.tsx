...
export default function AdminUsersPage() {
  ...
  const renderGrowthChart = () => {
    if (growthData.length === 0) {
      return <div className="text-center text-slate-400 py-8 text-sm">暂无增长数据</div>;
    }
    ...
    return (
      <svg viewBox={`0 0 ${chartW} ${chartH}`} className="w-full h-auto">
        ...
        {yTicks.map((tick, i) => {
          ...
          return (
            <g key={i}>
              <line x1={padX} y1={y} x2={chartW - padX} y2={y} stroke="#334155" strokeWidth="0.5" strokeDasharray="4,4" />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={<Users className="h-5 w-5" />} label="总用户数" value={stats?.total ?? '-'} color="text-blue-400" />
        <StatCard icon={<Crown className="h-5 w-5" />} label="会员数" value={stats?.members ?? '-'} color="text-amber-400" />
        <StatCard icon={<TrendingUp className="h-5 w-5" />} label="会员转化率" value={stats ? `${stats.conversionRate}%` : '-'} color="text-emerald-400" />
        <StatCard icon={<UserPlus className="h-5 w-5" />} label="本周新增" value={stats?.weeklyNew ?? '-'} color="text-purple-400" />
      </div>

      {/* 增长趋势 */}
      <Card className="bg-[#1E293B] border-slate-700">
        <CardHeader>
          <CardTitle className="text-white text-base">用户增长趋势（近30天）</CardTitle>
        </CardHeader>
        <CardContent>
          {renderGrowthChart()}
        </CardContent>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input
                ...
                className="pl-9 bg-[#0F172A] border-slate-600 text-white placeholder:text-slate-500"
                onKeyDown={e => { if (e.key === 'Enter') fetchUsers(1); }}
              />
            </div>
            <Select value={membershipFilter} onValueChange={v => setMembershipFilter(v === 'all' ? '' : v)}>
              <SelectTrigger className="w-[140px] bg-[#0F172A] border-slate-600 text-white">
            ...
            <Input
              ...
              className="w-[140px] bg-[#0F172A] border-slate-600 text-white placeholder:text-slate-500"
            />
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-slate-700 hover:bg-transparent">
                  ...
                  <TableHead className="text-slate-400">专业</TableHead>
                  <TableHead className="text-slate-400">年级</TableHead>
                  <TableHead className="text-slate-400">求职意向</TableHead>
                  <TableHead className="text-slate-400">会员</TableHead>
                  <TableHead className="text-slate-400">技能数</TableHead>
                  <TableHead className="text-slate-400">测评次数</TableHead>
              ...
              <TableBody>
                {users.length === 0 ? (
                  ...
                  >
                    ...
                    <TableCell className="text-slate-300">{user.major || '-'}</TableCell>
                    <TableCell className="text-slate-300">{user.grade || '-'}</TableCell>
                    <TableCell className="text-slate-300 max-w-[150px] truncate">{user.job_intention || '-'}</TableCell>
                    <TableCell>
                      {user.membership_type === 'member' ? (
                        ...
                        <Badge variant="outline" className="text-slate-400 border-slate-600">免费</Badge>
                      )}
                    </TableCell>
                    <TableCell className="text-slate-300">{user.skill_count}</TableCell>
                    <TableCell className="text-slate-300">{user.assessment_count}</TableCell>
      ...
      <Sheet open={!!selectedUserId} onOpenChange={open => { if (!open) { setSelectedUserId(null); setUserDetail(null); } }}>
        <SheetContent className="bg-[#0F172A] border-slate-700 text-white w-[560px] sm:max-w-[560px] overflow-y-auto">
          <SheetHeader>
            <SheetTitle className="text-white">用户详情 — #{selectedUserId}</SheetTitle>
            <SheetDescription className="text-slate-400">查看用户完整画像和行为数据</SheetDescription>
          </SheetHeader>
            ...
            <div className="mt-6 space-y-6">
              ...
              <DetailSection title={`技能 (${userDetail.skills.length})`} icon={<Target className="h-4 w-4" />}>
                {userDetail.skills.length === 0 ? (
                  <div className="text-slate-500 text-sm">暂无技能数据</div>
                ) : (
              ...
              <DetailSection title={`测评记录 (${userDetail.assessments.length})`} icon={<Activity className="h-4 w-4" />}>
                {userDetail.assessments.length === 0 ? (
                  <div className="text-slate-500 text-sm">暂无测评记录</div>
                ) : (
                      ...
                      return (
                        <div key={i} className="bg-slate-800/50 rounded-lg p-3 text-sm">
                          <div className="flex justify-between text-slate-300">
                            ...
                            <span className="text-slate-500">{a.created_at ? new Date(a.created_at as string).toLocaleDateString('zh-CN') : ''}</span>
                          </div>
              ...
              <DetailSection title={`匹配记录 (${userDetail.matches.length})`} icon={<Briefcase className="h-4 w-4" />}>
                {userDetail.matches.length === 0 ? (
                  <div className="text-slate-500 text-sm">暂无匹配记录</div>
                ) : (
                  <div className="space-y-2">
              ...
              <DetailSection title="行为统计" icon={<MessageSquare className="h-4 w-4" />}>
                {userDetail.behaviorStats.length === 0 ? (
                  <div className="text-slate-500 text-sm">暂无行为数据</div>
                ) : (
                  <div className="space-y-2">
                    {userDetail.behaviorStats.map((b, i) => (
                      <div key={i} className="flex justify-between text-sm">
                        <span className="text-slate-300">{b.event_type as string}</span>
                        <span className="text-blue-400 font-mono">{b.count as number}次</span>
                      </div>
                    ))}
                  </div>
                )}
