      ...
      <section className="bg-[#1E3A8A] border-y border-blue-700/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            {[
              { value: 'AI原生', label: 'DeepSeek大模型驱动，非套壳方案' },
              { value: '零部署负担', label: '云端SaaS，高校无需自建服务器' },
              { value: '按需定制', label: '支持学校定制专属面试题库与测评维度' },
            ].map(s => (
              <div key={s.label}>
                <div className="text-xl md:text-2xl font-bold text-white">{s.value}</div>
                <div className="text-sm text-blue-200/70 mt-2">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
