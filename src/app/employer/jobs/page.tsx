...
export default function JobPostsPage() {
  ...
  const statusLabels: Record<string, { label: string; color: string }> = {
    ...
    closed: { label: '已关闭', color: 'bg-gray-100 text-slate-500' },
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">管理岗位画像与候选人匹配</p>
        </div>
        <Link
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-slate-400 bg-white rounded-2xl border border-slate-100">
          ...
          <p className="text-base font-medium text-slate-500 mb-1">暂无岗位</p>
          <p className="text-sm text-slate-400 mb-4">创建你的第一个岗位画像，开始匹配候选人</p>
          <Link
            href="/employer/jobs/new"
          ...
          {jobs.map((job) => {
            ...
            return (
              <Link
                ...
                className="block bg-white rounded-xl border border-slate-100 hover:border-[#165DFF]/30 hover:shadow-md transition p-4"
              >
                <div className="flex items-center justify-between">
      ...
      {total > pageSize && (
        <div className="flex justify-center gap-2">
          <button
            ...
            disabled={page === 1}
            className="px-3 py-1.5 text-sm bg-white border border-slate-200 rounded-lg disabled:opacity-50 hover:bg-gray-50 transition"
          >
            上一页
          </button>
