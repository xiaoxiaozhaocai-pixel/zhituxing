...
5. category 从以下选择：校园生活、专业信息、就业情况、考研数据、社团活动、校园设施、学生文化、学术资源、食堂美食、其他
6. tags 用2-4个标签概括

输出格式（纯JSON数组，不要markdown包裹）：
[{"category":"校园生活","content":"花江校区图书馆考试周期间开放到晚上11点","tags":["图书馆","花江校区","考试周"]}]
...
async function extractWithDeepSeek(userMessage: string, assistantResponse: string): Promise<GuetKnowledgeItem[]> {
  ...
  try {
    ...
    return items.filter((item: GuetKnowledgeItem) =>
      item.category && item.content && item.tags && item.tags.length > 0
    );
  } catch (err) {
