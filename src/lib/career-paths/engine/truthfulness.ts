...
/** 团队成果写成个人成果（真实负责） */
// 防误伤：只有「集体成果」语境（团队/组内/公司/部门/我们/大家/全班/全校）才需检查是否标了个人份额；
// 纯个人获奖/成就（「我获得校级奖学金」）不属于「抢功」，不应触发。
const TEAM_AS_OWN = [
  { re: /(团队|组内|公司|部门|我们|大家|全班|全校).{0,15}(获得|拿下|实现|完成|达成).{0,15}(奖|成绩|第一|突破)/, why: '把团队/组织成果写成集体的成绩，却没有标明你个人的贡献份额，容易被追问「你具体做了什么」而答不上来，成为抢功。', fix: '写明个人份额：团队取得X，我具体负责Y，贡献Z。例：「团队获X奖，我负责Y环节，完成Z」。' },
];
...
export function walkTruthfulness(experience: string, targetJob?: string): TruthfulnessReport {
  ...
  for (const rule of ALL_RULES) {
    ...
    if (m) {
      ...
      visited.add(key);
      // ========== 防误伤（守四真但不冤枉真实经历） ==========
      // 1) VAGUE_OWNERS「参与/协助」：若句中已有明确职责动词（负责/完成/主导/承担/统筹）或第一人称「我」，
      //    则「参与/协助」不是模糊无主，跳过该条。
      if (rule.re === VAGUE_OWNERS[0].re && /负责|完成|主导|承担|统筹|我/.test(input)) {
        continue;
      }
      // 2) TEAM_AS_OWN「团队成果写成个人」：若句中已明确个人贡献份额（我负责/我承担/我完成/本人/我主导/我作为），
      //    说明已标明个人边界，不是抢功，跳过该条。
      if (rule.re === TEAM_AS_OWN[0].re && /(我负责|我承担|我完成|我主导|我作为|本人负责|本人完成|我参与其中|我负责其中|我完成其中|我主要)/.test(input)) {
        continue;
      }
      // 3) LEAD_WITHOUT_PROOF「主导/独立但无成果」：若是学生课业/学习场景（课程/课设/期末/作业/毕业设计/实训/小组/比赛/竞赛/大赛），
      //    独立完成课业是常态且可背调（有成绩单/作业），不构成「有承诺无条件」，跳过该条。
      if (rule.re === LEAD_WITHOUT_PROOF[0].re && /(课程|课设|期末|毕业设计|课程设计|作业|实训|小组|社团|比赛|竞赛|大赛|项目设计|课程项目)/.test(input)) {
        continue;
      }
      // 4) UNSUPPORTED_NUMBER「数字无来源」：若句中已给出数据口径/来源（数据来自/来源于/来源/系统导出/统计/报告/显示/口径/依据/记录），
      //    则是可背调的数据，不算「无来源夸大」，跳过该条。
      if (rule.re === UNSUPPORTED_NUMBER[0].re && /(数据来(自|源)|来源于|来源|系统导出|统计|报告|显示|口径|依据|记录|截图|后台)/.test(input)) {
        continue;
      }
